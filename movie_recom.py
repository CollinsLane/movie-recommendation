...
def load_data():
    return pd.DataFrame({
        ...
        'Image_URL': [
            ...
            'https://upload.wikimedia.org/wikipedia/en/6/6e/Mad_Max_Fury_Road_poster.jpg',
            'https://upload.wikimedia.org/wikipedia/en/0/0f/Everything_Everywhere_All_at_Once_poster.jpg'
        ],
        'Genres': [
            ...
            ['Sci-Fi', 'Adventure'], ['Action', 'Adventure'], ['Action', 'Comedy']
        ],
        'Rating': [9.0, 8.8, 8.6, 8.4, 7.4, 8.7, 8.0, 8.1, 8.9, 8.8, 8.2, 8.5],
        'Description': [
            "Batman faces the Joker in Gotham.", "A thief steals secrets from dreams.",
            "Explorers travel through a wormhole.", "The Avengers assemble once more.",
            "An ex-hitman seeks vengeance.", "A hacker learns the true nature of reality.",
            "A noble family entangled in a war for spice.", "A blade runner unearths a long-buried secret.",
            "The story of J. Robert Oppenheimer.", "Paul Atreides unites the desert people.",
            "In a wasteland, a woman and a warrior fight for survival.", 
            "An aging woman discovers the multiverse."
        ]
    })
...
def calculate_matches(df, selected_genres, min_rating, search_query):
    if search_query:
        ...
        if not results.empty:
            ...
            results['Matches'] = [["Direct Title Match"]] * len(results)
        else:
    ...
    df = df.copy()
    df['Matches'] = df['Genres'].apply(lambda x: list(set(x) & set(selected_genres)))
    df['Match_Score'] = df['Matches'].apply(len)
    max_possible = len(selected_genres)
    
    results = df[(df['Match_Score'] > 0) & (df['Rating'] >= min_rating)]\
              .sort_values(by=['Match_Score', 'Rating'], ascending=False)
    return results, max_possible

# ==================== 4. RENDER CARD (NOW BULLETPROOF) ====================
def render_movie_card(row, max_possible):
    ...
    with st.container(border=True):
        ...
        # Title
        st.markdown(f"""
        <div style="font-weight:700; font-size:1.2rem; line-height:1.3; min-height:62px;">
            {row['Title']}
        </div>
        """, unsafe_allow_html=True)
        ...
        # Matched genres
        matched_text = "Direct Title Match" if row['Matches'][0] == "Direct Title Match" else ", ".join(row['Matches'])
        st.markdown(f"""
...
with st.sidebar:
    ...
    selected_genres = st.pills("Select tags below:", options=all_genres, selection_mode="multi",
                               default=["Action", "Sci-Fi"], label_visibility="collapsed")
    
    if st.button("🎲 Surprise Me", use_container_width=True):
        selected_genres = random.sample(all_genres, k=random.randint(2, 4))
        st.rerun()
    ...
    st.info("**Neural Engine** filters → matches genres → ranks by score + IMDb rating.")
...
col1.metric("📊 Total Databanks", len(df))
...
if search_query:
    ...
    <div class="logic-box" style="border-color:#a855f7; background:rgba(168,85,247,0.1);">
        <b>🔥 DIRECT SEARCH OVERRIDE</b><br>Showing results for <code>{search_query}</code>
    </div>
    """, unsafe_allow_html=True)
elif selected_genres:
    st.markdown(f"""
    <div class="logic-box">
        <b>Selection Logic:</b> Rating ≥ {min_rating} • Contains at least one of: <code>{', '.join(selected_genres)}</code>
    </div>
    """, unsafe_allow_html=True)

# Render grid
if not (selected_genres or search_query):
    st.info("👋 Select genres or search a title to activate the Neural Engine.")
elif results.empty:
    st.warning("⚠️ No matches found. Try lowering the Quality Floor or broadening genres.")
else:
...
st.markdown(
    ...
    "🧬 AI Movie Engine • Final Year Project • errorHQ © 2026"
    "</p>",
    unsafe_allow_html=True
)
